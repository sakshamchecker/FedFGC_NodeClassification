...
def execute(args, coarsen, path, priv):
    ...
    attack_train_loader, attack_test_loader=data_creator(target_model=net, shadow_model=shadow_net, target_train=train_loader, target_test=val_loader, shadow_train=shadow_train, shadow_test=shadow_test, device=device)
    ...
    attack_train(model=attack_net, trainloader=attack_train_loader, testloader=attack_test_loader, device=device, criterion=criterion, optimizer=optimizer, epochs=args.at_epochs, steps=0)
    ...
    for original in [True, False]:
        ...
        try:
            ...
            data = pd.DataFrame(columns=["Method", "Coarsen","Privacy","Original","Loss","Accuracy","AUROC","Precision","Recall", "Final_f_score"])
        data=pd.concat([data, pd.Series(['AllData', coarsen,priv,original, tranc_floating(test_loss), tranc_floating(test_accuracy), tranc_floating(final_auroc), tranc_floating(final_precision), tranc_floating(final_recall), tranc_floating(final_f_score)], index=data.columns).to_frame().T], ignore_index=True)
...
def execute_FL(args, coarsen, path, idxs=None, priv=False):
    ...
    train_loaders_cr, train_loaders_non, test_loaders, shadow_train, shadow_test, num_node_features, num_classes=load_clients_data(args.data, args.ncl, args.tr_ratio, cr=coarsen, cr_ratio=args.cr_ratio, sh_ratio=args.sh_ratio)
    ...
    shadow_net=copy.deepcopy(mod)
    print('-- Training Attack Model --')
    attack_train_loader, attack_test_loader=data_creator(target_model=None, shadow_model=shadow_net, target_train=None, target_test=None, shadow_train=shadow_train, shadow_test=shadow_test, device=device)
    attack_net=Net(num_classes=num_classes)
    criterion = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(attack_net.parameters(), lr=0.01,weight_decay=0.0001)
    attack_train(model=attack_net, trainloader=attack_train_loader, testloader=attack_test_loader, device=device, criterion=criterion, optimizer=optimizer, epochs=args.at_epochs, steps=0)
    ...
    try:
        ...
        print("")
    def client_fn(cid):
        return FlowerClient(cid, net, train_loaders_non[int(cid)],train_loaders_cr[int(cid)], test_loaders, args.epochs, path=path, state=coarsen, device=device, args=args, dp=priv, priv_budget=args.priv_budget, attack_net=attack_net) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cr_ratio', default=0.3, type=float, help='coarsen ratio')
    parser.add_argument('--priv_budget', default=0.15, type=float, help='privacy budget')
    parser.add_argument('--lr' , default=0.1, type=float, help='learning rate')
    parser.add_argument('--sh_ratio', default=0.5, type=float, help='shadow ratio')
    parser.add_argument('--at_epochs', default=100, type=int, help='number of attack epochs')
    parser.add_argument('--central', default="True", type=str)
    parser.add_argument('--fed', default="True", type=str)
