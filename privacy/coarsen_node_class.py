...
def experiment(lambda_param,beta_param,alpha_param,gamma_param,C,X_tilde,theta,X,thresh, cr_ratio,y):
      p = X.shape[0]
      k = int(p*cr_ratio)
      n = X.shape[1]
...
def coarsening(adj,X,y,lambda_param,beta_param,gamma_param,alpha_param, cr_ratio):
    ...
    try:
      X2,C2=experiment(lambda_param,beta_param,alpha_param,gamma_param,C,X_tilde,theta,X1,thresh, cr_ratio, y)
    except:
      return [adj,X]
    C_tr=torch.transpose(C2,0,1)
    theta_c=C_tr@theta@C2
    y=C_tr@y.argmax(dim=1)
    adjtemp = -theta_c
...
def coarsen_a_dataset(client_train_dataset, coarsen_params, batch_size, cr_ratio):
    training_graphs=[]
    adj=to_dense_adj(data.edge_index)[0]
    X=data.x
    # adj1,X1=coarsening(adj,X,0.1,1,50,50)
    adj1,X1, y1=coarsening(adj,X,y,coarsen_params[0],coarsen_params[1],coarsen_params[2],coarsen_params[3], cr_ratio)
    # print(adj1.shape)
    ...
    if adj1.shape!=adj.shape:
        ...
        g=Data(x=X1,edge_index=temp[0],edge_attr=temp[1],y=y1)
        del g
        del temp
        del A
    del adj1 
...
dataset = Planetoid(root='./data/Cora', name='Cora', split="random")
